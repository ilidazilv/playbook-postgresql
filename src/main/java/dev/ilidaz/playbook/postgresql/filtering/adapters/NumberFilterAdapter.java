...
public abstract class NumberFilterAdapter<T extends Number & Comparable<T>, F extends NumberFilters<T>> extends BaseFilterAdapter<BaseFilters<T>, T> {

    protected Class<F> filtersClass;

    protected NumberFilterAdapter() {
        super();
    ...
    public List<Predicate> generatePredicates(F filters, CriteriaBuilder cb, Path<T> path) {
        ...
        if (filters.getGreaterThan() != null) {
            predicates.add(cb.gt(path, filters.getGreaterThan()));
        }
        ...
        if (filters.getLowerThan() != null) {
            predicates.add(cb.le(path, filters.getLowerThan()));
        }
